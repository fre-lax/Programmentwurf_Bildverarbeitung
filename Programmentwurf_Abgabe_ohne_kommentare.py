...
def run(image, result, settings=(100, 100)):
    ...
    final_selction, box = find_final_rectangle(
        merged_contours_img, image, settings, scale_factor
    )
    ...
    result.append(
        {
            "name": f"scaled to 50% - runtime: {round((time.time()-start)*1000)} ms",
            "data": scaled,
        }
    )
    return result
...
def find_contours(black_white_image, settings, scale_factor):
    contours, _ = cv2.findContours(
        black_white_image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
    )
    contour_image = np.zeros(
        (black_white_image.shape[0], black_white_image.shape[1], 3),
        dtype=np.uint8,
    )
...
def find_final_rectangle(merged_contours_img, original_image, settings, scale_factor):
    ...
    merged_contours_img, contours = find_contours(
        merged_contours_img, settings, scale_factor
    )
...
def rotation_korrigieren(cropped_image, svm):
    zwei_prozent_streifen = cropped_image[
        int(cropped_image.shape[0] / 100 * 90) : int(cropped_image.shape[0] * 92 / 100),
        :,
        :,
    ]
...
def verabeiten(file, output_dir):
    ...
    print(
        f'Verarbeitung von {file.split("/")[-1]} dauerte {(time.time()-start)*1000} ms.'
    )
...
if __name__ == "__main__":
    ...
    if files is None or output_dir == "":
        exit()
    tasks = [(file, output_dir) for file in files]
    with multiprocessing.Pool(
        initializer=init_worker, processes=multiprocessing.cpu_count()
    ) as pool:
        results = pool.starmap(verabeiten, tasks)
    pool.close()
    pool.join()
    print(
        f"Verarbeitung von {len(files)} Bildern dauerte {time.time()-start_tick} Sekunden."
    )
    print(
        f"Durchschnittliche Zeit pro Bild: {(time.time()-start_tick)/len(files)} Sekunden."
    )
